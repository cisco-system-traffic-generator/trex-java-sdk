...
public class TRexClientTest {
  ...
  public void setL3Test() {
    ...
    try {
      ...
      boolean result =
          client.setL3Mode(port.getIndex(), nextHopMac, "192.168.9.27", "192.168.9.28");
  ...
  public void getCapturesTest() {
    ...
    Optional<CaptureInfo> monitorInfoResult =
        Arrays.stream(activeCaptures.get())
  ...
  public void startRecorderTest() {
    ...
    TRexClientResult<CaptureInfo[]> activeCaptures = client.getActiveCaptures();
    Optional<CaptureInfo> recordMonitor =
        Arrays.stream(activeCaptures.get())
  ...
  public void captureRecordStopTest() {
    ...
    TRexClientResult<CaptureMonitorStop> stopResult =
        client.captureMonitorStop(result.get().getCaptureId());
    ...
    Optional<CaptureInfo> stoppedMonitor =
        Arrays.stream(activeCapturesResult.get())
  ...
  public void fetchCapturedPKtsTest() throws InterruptedException {
    ...
    TRexClientResult<CapturedPackets> capturedPktsResult =
        client.captureFetchPkts(result.get().getCaptureId(), 10);
  ...
  public void getAllStreamsTest() {
    ...
    for (Stream str : allStreams) {
      Optional<Stream> found =
          allStreams.stream().filter(stream -> stream.getId().equals(str.getId())).findFirst();
      if (!found.isPresent()) {
  ...
  public void getActivePgIds() throws InterruptedException {
    ...
    Assert.assertArrayEquals(
        new int[] {25, 255, 434}, Arrays.stream(activePgids.getFlowStats()).sorted().toArray());
    Assert.assertArrayEquals(
        new int[] {78, 1315, 9801}, Arrays.stream(activePgids.getLatency()).sorted().toArray());
  }
  ...
  public void getPgIdStats() throws InterruptedException {
    ...
    for (int pgId : Arrays.asList(333, 444)) {
      ...
      Assert.assertEquals(
          flowStats.get(pgIdStr).getRb().get(port1), flowStats.get(pgIdStr).getTb().get(port0));
      Assert.assertEquals(
          flowStats.get(pgIdStr).getRbs().get(port1), flowStats.get(pgIdStr).getTbs().get(port0));
      Assert.assertEquals(
          flowStats.get(pgIdStr).getRp().get(port1), flowStats.get(pgIdStr).getTp().get(port0));
      Assert.assertEquals(
          flowStats.get(pgIdStr).getRps().get(port1), flowStats.get(pgIdStr).getTps().get(port0));
    }
  }
  ...
  private static StreamMode createStreamMode() {
    return new StreamMode(
        ...
        0,
        1000.0,
        new StreamModeRate(StreamModeRate.Type.pps, 1000.0),
  ...
  public static EthernetPacket buildUdpPacket() {
    ...
    tosBuilder.precedence(IpV4TosPrecedence.ROUTINE);
    ipv4Builder
        .srcAddr(srcAddr)
        .dstAddr(dstAddr)

...
public class TRexClient {
  ...
  public List<String> getSupportedCommands() {
    ...
    JsonElement response = new JsonParser().parse(json);
    JsonArray cmds =
        response.getAsJsonArray().get(0).getAsJsonObject().get("result").getAsJsonArray();
    ...
    try {
      ...
      while (steps > 0) {
        ...
        if (pkts.size() > 0) {
          ...
          if (arpPacket != null) return arpPacket.getHeader().getSrcHardwareAddr().toString();
        }
      }
  ...
  private static ArpPacket getArpPkt(Packet pkt) {
    if (pkt.contains(ArpPacket.class)) return pkt.get(ArpPacket.class);
    try {
  ...
  private static EthernetPacket buildArpPkt(
    ...
    if (vlan.getTags().size() != 0) payload = buildVlan(arpBuilder, vlan);
  ...
  private static Pair<EtherType, Packet.Builder> buildVlan(

...
class CPacketData {
    ...
    CPacketData add(CPacketData cPacketData) {
        ...
        throw new IllegalStateException(String.format("input packet data direction: %s is different from current direction: %s",
...
class CpcapReaderHelp {
    ...
    /**
     * analyze pcap file.after analyzing,it will get all data from pacp file
     * please use this method before invoke get methods
     */
    void analyze() {
        if (analyzed) {
            return;
        ...
        while (true) {
            ...
            if (clientIp == null) {
                ...
                if (clientIp.equals(ipHeader.getSrcAddr()) && serverIp.equals(ipHeader.getDstAddr())) {
                    direction = SideType.Client;
                } else if (serverIp.equals(ipHeader.getSrcAddr()) && clientIp.equals(ipHeader.getDstAddr())) {
                    direction = SideType.Server;
                } else {
                    this.fail(String.format("Only one session is allowed in a file. Packet %s is from different session", index));
                }
            }
            ...
            if (tcp != null) {
                ...
                if (l4Header.getSyn()) {
                    ...
                    if (l4Header.getAck()) {
                        ...
                        if (state == states.get(INIT)) {
                            ...
                            this.fail(String.format("Packet #%s is SYN+ACK, but there was already SYN+ACK in cap file", index));
                        }
                    ...
                    //SYN - no ACK. Should be first packet client->server
                        ...
                        if (state > states.get(SYN)) {
                            this.fail(String.format("Packet #%s is TCP SYN, but there was already TCP SYN in cap file", index));
                        } else {
                ...
                if (!(l4Type == null || l4Type.equals(TCP))) {
                    this.fail(String.format("PCAP contains both TCP and %s. This is not supported currently.", l4Type));
                }
                l4Type = TCP;
            ...
            //UDP scenario
                ...
                if (!(l4Type == null || l4Type.equals(UDP))) {
                    this.fail(String.format("PCAP contains both UDP and %s. This is not supported currently.", l4Type));
                }
                l4Type = UDP;
            ...
            if (tcp != null && !tcp.getHeader().getSyn()) {
                if (tcp.getHeader().getSrcPort().valueAsInt() == this.srcPort) {
                    if (expClientSeq != tcp.getHeader().getSequenceNumberAsLong()) {
                        this.fail(String.format("TCP seq in packet %s is %s. We expected %s. Please check that there are no packet loss or retransmission in cap file",
